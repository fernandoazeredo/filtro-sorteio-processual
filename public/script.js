window.addEventListener("DOMContentLoaded", () => {
  ...
  const isCnj=v=>/^\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}$/.test(String(v??"").trim());
  const isProcessRow=r=>String(r?.Cliente??"").trim()&&(isNumericId(r.ID)||isNumericId(r.ID1)||isCnj(r["Número de CNJ"]));
  const key=r=>isCnj(r["Número de CNJ"])?`CNJ:${String(r["Número de CNJ"]).trim()}`:isNumericId(r.ID)?`ID:${String(r.ID).trim()}`:isNumericId(r.ID1)?`ID1:${String(r.ID1).trim()}`:`R:${norm(r.Cliente)}|${norm(r.Pasta)}`;
  ...
  if($("filterList")&&![...$("filterList").options].some(o=>norm(o.value)==="ALEATORIO")){const o=document.createElement("option");o.value="Aleatório";$("filterList").appendChild(o);}

  function mergeSheets(items){const map=new Map();let total=0;for(const {name,rows} of items)for(const src of rows){total++;const k=key(src);if(!map.has(k))map.set(k,{...src,__sheets:new Set()});const t=map.get(k);for(const c of Object.keys(src))if((t[c]===""||t[c]==null)&&src[c]!==""&&src[c]!=null)t[c]=src[c];t.__sheets.add(name);}duplicateCount=total-map.size;return[...map.values()].map(r=>({...r,__sheets:[...r.__sheets]}));}
  function detectCols(items){const out=[],seen=new Set();for(const {rows} of items)for(const r of rows.slice(0,30))for(const c of Object.keys(r))if(!c.startsWith("__")&&!seen.has(c)){seen.add(c);out.push(c);}if(!seen.has("Sorteado Para"))out.push("Sorteado Para");return out;}
  function rawMatch(r,g){const type=norm(r.Tipo),dec=norm(r["Última Decisão"]),crit=norm(r.Critério),s=(r.__sheets||[]).map(norm);if(g==="EF")return type==="EF"||s.includes("EF");if(g==="ED")return type==="ED"||s.includes("ED");if(g==="EP")return type==="EP"||s.includes("EP");if(g==="IMPROCEDENTE")return dec==="IMPROCEDENTE"||s.includes("IMPROCEDENTE");if(g==="NADJA/FLAVIO")return crit.includes("NADJA - SOLICITOU FICAR FLAVIO")||crit.includes("NADJA/FLAVIO")||s.some(x=>x.includes("NADJA - SOLICITOU FICAR FLAVIO"));if(g==="NADJA")return crit==="NADJA"||s.includes("NADJA");if(g==="COMPROMETIDO")return crit.includes("COMPROMET")||crit.includes("COMPREMET")||s.some(x=>x.includes("COMPROMET"));if(g==="FLAVIO")return crit.includes("CLIENTE FLAVIO")||crit==="FLAVIO"||s.some(x=>x.includes("CLIENTES FLAVIO"));if(g==="ANA")return crit.includes("CLIENTE ANA PAULA")||crit==="ANA"||s.some(x=>x.includes("CLIENTE ANA PAULA"));return false;}
  ...
  function assignRandom(g){
    ...
    for(let i=0;i<ordered.length;i++){
      ...
      if(mustFlavio)toFlavio=true;
        ...
        const scoreFlavio=Math.abs((fCount+1)/processedCount-target)+(processedValue>0?Math.abs((fValue+value)/processedValue-target):0);
        const scoreAna=Math.abs(fCount/processedCount-target)+(processedValue>0?Math.abs(fValue/processedValue-target):0);
        if(Math.abs(scoreFlavio-scoreAna)<1e-12){
          const finalValueIfF=fValue+value;
          const finalValueIfA=fValue;
    ...
    while(improved&&passes<3){
      ...
      const fs=p.filter(r=>partner(r)==="Flávio");
      const as=p.filter(r=>partner(r)==="Ana");
      let best=null,bestGain=0;
      const currentDiff=Math.abs(fValue-targetValue);
      for(const fr of fs){
        const fv=Math.max(0,parseBRL(fr["Valor da causa"]));
        for(const ar of as){
          const av=Math.max(0,parseBRL(ar["Valor da causa"]));
          const newValue=fValue-fv+av;
          const gain=currentDiff-Math.abs(newValue-targetValue);
          if(gain>bestGain+1e-9){bestGain=gain;best={fr,ar,newValue};}
        }
      }
  ...
  $("excelFile").onchange=e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>{try{const wb=XLSX.read(new Uint8Array(ev.target.result),{type:"array",cellStyles:true}),items=[];let ignored=0,rawProcesses=0;for(const name of wb.SheetNames){const all=XLSX.utils.sheet_to_json(wb.Sheets[name],{defval:"",raw:true}),rows=all.filter(isProcessRow);ignored+=all.length-rows.length;rawProcesses+=rows.length;if(rows.length)items.push({name,rows});}if(!items.length)throw Error("Nenhuma aba com processos válidos foi encontrada.");available=detectCols(items);master=mergeSheets(items);clearAssignments();buildGroups();filtered=[...master];activeFilter="";selected=DEFAULT_COLS.filter(c=>available.includes(c));if(!selected.includes("Sorteado Para"))selected.push("Sorteado Para");batchDone=false;colBtn.disabled=previewBtn.disabled=false;packageBtn.disabled=true;render(filtered);summaryUI(filtered);modal("Planilha carregada",`<p><strong>${master.length}</strong> processos únicos válidos foram identificados.</p><p>Registros de processos lidos nas abas: ${rawProcesses}.<br>Duplicidades consolidadas entre abas: ${duplicateCount}.<br>Linhas auxiliares/resumos ignoradas: ${ignored}.</p><p class="fsp-ok">Todos os ${master.length} processos únicos serão classificados em exatamente um grupo. <strong>Sorteado Para</strong> começa vazia.</p>`);}catch(err){modal("Erro ao ler planilha",`<p>${esc(err.message)}</p>`);reset();}};reader.readAsArrayBuffer(file);};
  ...
  function executeBatch(){clearAssignments();assignFixed();RANDOM_GROUPS.forEach(assignRandom);const assigned=master.filter(r=>partner(r)).length;if(assigned!==master.length){batchDone=false;packageBtn.disabled=true;return modal("Falha de auditoria",`<p class="fsp-warn">Foram atribuídos ${assigned} de ${master.length} processos. Os relatórios foram bloqueados para evitar resultado incompleto.</p>`);}batchDone=true;packageBtn.disabled=false;filtered=[...master];activeFilter="";render(filtered);summaryUI(master);modal("Sorteios concluídos",`${resultHtml()}<p class="fsp-ok"><strong>${assigned} de ${master.length}</strong> processos receberam uma atribuição em Sorteado Para.</p>`);}
  $("drawBtn").onclick=()=>{if(!master.length)return modal("Atenção","<p>Selecione a planilha primeiro.</p>");const grouped=[...groupMap.values()].reduce((a,b)=>a+b.length,0);if(grouped!==master.length)return modal("Sorteio bloqueado",`<p class="fsp-warn">A auditoria encontrou ${master.length-grouped} processo(s) sem grupo.</p>`);modal("Confirmar processamento",`${previewHtml()}<p><strong>Confirmar a distribuição equilibrada por quantidade e valor?</strong></p>`,{confirmText:"Executar",onConfirm:executeBatch});};
