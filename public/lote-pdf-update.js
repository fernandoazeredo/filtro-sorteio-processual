(() => {
  ...
  const QTARGET=.60, VTARGET=.60, SWAPS=40, REFINES=6;
  ...
  const pct=(a,b)=>b?`${((a/b)*100).toFixed(2)}%`:"0.00%", clamp=(v,a,b)=>Math.min(b,Math.max(a,v));
  ...
  function balanced(rs,target){
    if(!rs.length)return;rs.forEach(r=>r["Sorteado Para"]="");const q=quota(rs),total=rs.reduce((s,r)=>s+val(r),0),ordered=[...rs].sort((a,b)=>val(b)-val(a)||String(a.Cliente||"").localeCompare(String(b.Cliente||""),"pt-BR"));
    let fc=0,ac=0,fv=0,pv=0;
    ordered.forEach((r,i)=>{const v=val(r),pc=i+1;pv+=v;let f;if(ac>=q.a)f=true;else if(fc>=q.f)f=false;else{const sf=Math.abs((fc+1)/pc-QTARGET)+(pv?Math.abs((fv+v)/pv-target):0),sa=Math.abs(fc/pc-QTARGET)+(pv?Math.abs(fv/pv-target):0);f=sf<=sa}r["Sorteado Para"]=f?"Flávio":"Ana";if(f){fc++;fv+=v}else ac++});
    const tv=total*target;
    for(let pass=0;pass<SWAPS;pass++){
      const fr=rs.filter(r=>r["Sorteado Para"]==="Flávio"),ar=rs.filter(r=>r["Sorteado Para"]==="Ana"),cur=Math.abs(fv-tv);let best=null,gain=0;
      for(const f of fr){const x=val(f);for(const a of ar){const y=val(a),nf=fv-x+y,ng=cur-Math.abs(nf-tv);if(ng>gain+1e-9){gain=ng;best=[f,a,nf]}}}
      if(!best)break;best[0]["Sorteado Para"]="Ana";best[1]["Sorteado Para"]="Flávio";fv=best[2];
    }
  }

  function allocate(rows){
    for(const [g] of FILTERS)if(!RANDOM.has(g))fixed(rows.filter(r=>r.__group===g),g);
    const total=rows.reduce((s,r)=>s+val(r),0),rr=rows.filter(r=>RANDOM.has(r.__group)),rt=rr.reduce((s,r)=>s+val(r),0),fixedF=rows.filter(r=>!RANDOM.has(r.__group)&&r["Sorteado Para"]==="Flávio").reduce((s,r)=>s+val(r),0),goal=total*VTARGET;
    let target=rt?clamp((goal-fixedF)/rt,0,1):VTARGET,best=Infinity,snap=null;
    for(let k=0;k<REFINES;k++){
      for(const [g] of FILTERS)if(RANDOM.has(g))balanced(rows.filter(r=>r.__group===g),target);
      const s=summary(rows),err=s.fv-goal,d=Math.abs(err);if(d<best-.0001){best=d;snap=rows.map(r=>r["Sorteado Para"])}if(!rt||d<=.01)break;const nt=clamp(target-err/rt,0,1);if(Math.abs(nt-target)<1e-12)break;target=nt;
    }
    if(snap)rows.forEach((r,i)=>r["Sorteado Para"]=snap[i]||"");const missing=rows.filter(r=>!r["Sorteado Para"]);if(missing.length)throw Error(`${missing.length} processo(s) ficaram sem atribuição.`);
  }
  ...
  async function run(btn){const input=document.getElementById("excelFile"),file=input?.files?.[0];if(!file)return alert("Selecione primeiro a planilha do novo sorteio.");if(!window.XLSX||!window.jspdf?.jsPDF)return alert("Os componentes de Excel/PDF ainda não foram carregados. Atualize a página e tente novamente.");if(!confirm("Executar agora o sorteio completo de todos os filtros, aplicar os grupos fixos e baixar todos os relatórios em um único ZIP?"))return;const old=btn.textContent;btn.disabled=true;btn.textContent="Processando sorteio e relatórios...";try{await zipLib();const wb=XLSX.read(new Uint8Array(await file.arrayBuffer()),{type:"array",cellStyles:true}),rows=readRows(wb);allocate(rows);const zip=new JSZip();let n=0;for(const f of FILTERS){const rs=rows.filter(r=>r.__group===f[0]);if(!rs.length)continue;n+=rs.length;zip.file(`${f[2]}.pdf`,filterPdf(f,rs).output("arraybuffer"))}if(n!==rows.length)throw Error(`${rows.length-n} processo(s) ficaram fora dos relatórios.`);zip.file("11 - RESUMO CONSOLIDADO.pdf",summaryPdf(rows).output("arraybuffer"));const blob=await zip.generateAsync({type:"blob"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`SORTEIO_COMPLETO_RELATORIOS_${new Date().toISOString().slice(0,10)}.zip`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);const s=summary(rows);alert(`Sorteio completo concluído. ${rows.length} processos foram atribuídos. Rateio consolidado por valor: Flávio ${pct(s.fv,s.tv)} / Ana ${pct(s.av,s.tv)}.`)}catch(e){alert(`Erro no sorteio completo: ${e.message}`)}finally{btn.textContent=old;btn.disabled=!input.files?.[0]}}
  addEventListener("DOMContentLoaded",()=>{const b=document.getElementById("packageBtn"),i=document.getElementById("excelFile");if(!b||!i)return;b.onclick=null;b.textContent="Baixar Relatórios do Lote";b.title="Executa o sorteio completo, equilibra cada filtro e aproxima o consolidado geral de 60% / 40%, sem dividir nenhum processo.";b.addEventListener("click",()=>run(b));const sync=()=>b.disabled=!i.files?.[0];i.addEventListener("change",()=>setTimeout(sync,0));new MutationObserver(()=>{if(i.files?.[0]&&b.disabled&&b.textContent!=="Processando sorteio e relatórios...")b.disabled=false;if(!i.files?.[0]&&!b.disabled)b.disabled=true}).observe(b,{attributes:true,attributeFilter:["disabled"]});sync()});
})();
